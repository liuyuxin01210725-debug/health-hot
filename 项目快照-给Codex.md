...
- 收集：`python3 collect.py [信源名]`，或全局 skill **health-collect**（说「更新健康站 / 收集 X 最近的」）。
- 改完推送：`git add -A && git commit && git push`（约 1 分钟 Pages 自动更新）。
- 决策记录：`健康知识库/wiki/项目决策/health-hot上线决策-20260531.md`。

**已完成**：浅色健康风 UI · 22 条核验 · 每条独立详情页(`/claims/<slug>.html`) · 发布闸门(过 codex 对抗审查) · PubMed 锚点 · 信源分层 role(anchor/discovery/radar) · 证据按 PublicationType 结果级 · DOI/同节目去重 · 双链接(discovery_source_url + evidence_source_urls)。

**待办（都不挡用）**：
1. 12 条播客类里 **8 条 evidence 仍"待标注"**——逐条去 PubMed 找背后的真研究填进 `evidence_source_urls`（真研究活，别编 PMID）。
2. 锚点 PubMed 查询偏宽，会带跑题结果——收集锚点内容时把 query 收窄到本站主题。
3. 跨类 `tags` + 主题集合页；独立 `confidence` 字段。
4. 每周无人值守自动化：需 `gh auth refresh -s workflow` 授权 + LLM API key 作 secret。
5. 机构消费页（NIH ODS / 卫健委 / 丁香）无 RSS，按需在 `evidence_source_urls` 手动引用。
...
## 3. 数据模型（item 字段）
- 基本：`title` `source` `source_url`(http(s)) `category` `evidence`(rct/meta/guideline/observational/expert/blogger) `featured`(bool) `rank`(int) `summary`
- 结构化：`slug`(限 `^[a-z0-9]+(-[a-z0-9]+)*$`) `status`(须=reviewed) `conclusion` `population` `caveats`
- 双链接：`discovery_source_url`(在哪听到) + `evidence_source_urls`(凭什么核验，list)
- 日期：`date`(feed 日，闸门校验不得未来) / `source_published_at`(原文日，可预排未来，仅展示且标注) / `reviewed_at`(复核日)

## 4. 信源角色
anchor=核验锚点(PubMed 系统综述/Cochrane/指南，可定结论) · discovery=选题雷达(播客，只生成待核验问题) · radar=趋势雷达(博主/泛主题)。同节目多 feed 标 `group` 去重。

## 5. 7 铁律
证据分级 / 引用追溯 / 不给剂量 / 医疗免责 / 不暗示治疗 / 标注不确定 / 只摘要+回链。

## 6. 发布闸门（build.py `validate()`，经 codex 硬化）
拦截：缺/非串必填、类型错(featured/rank/evidence/category)、slug 非白名单(防 `../` 穿越与注入)、source_url 非 http(s)(防 `javascript:`/`data:`)、status≠reviewed、date/reviewed_at 非 ISO 或未来、slug/URL 重复、双链接字段类型错。渲染走临时目录→原子替换(崩溃不毁已上线 docs)。详情页有 SEO(description/og/canonical)。

## 7. 已验证（本轮 codex 复审后）
14 个攻击输入喂闸门：正常放行、13 攻击全拦；22 条详情页 0 断链；双链接三种情况渲染正确(源即依据/节目+真研究/节目+待标注)；collect.py 冒烟正常。

## 8. 设计决策
浅色健康风(非 AI HOT 深色)；知识库优先 + 轻量更新；信源求权威不求量；PubMed 作证据地基(免费)。
...
```json
{
  "_说明": "健康信源清单。type: youtube(channel_id)/rss(url)/pubmed(query)。",
  "_role": "anchor=核验锚点(系统综述/指南/Cochrane，可定结论) | discovery=选题雷达(播客，只生成待核验问题) | radar=趋势雷达(博主/泛主题，不直接定结论)。",
  "_group": "同一节目的多个 feed 标同一 group，collect.py 按 group+标题去重，避免一期内容收两次。",
  "_pubmed_evidence": "PubMed 条目的 evidence 由 collect.py 读 PublicationType 结果级判定(rct/meta/guideline/observational)，源里的 evidence 仅占位 unknown。",
  "sources": [
    {"name": "PubMed·Cochrane系统综述", "type": "pubmed", "role": "anchor", "query": "Cochrane Database Syst Rev[jour] AND (exercise OR nutrition OR diet OR sleep OR supplement OR longevity OR vitamin)", "category": "综述", "evidence": "unknown"},
    {"name": "PubMed·健康指南", "type": "pubmed", "role": "anchor", "query": "guideline[pt] AND humans[mh] AND (exercise OR nutrition OR diet OR sleep OR obesity OR hypertension OR diabetes)", "category": "指南", "evidence": "unknown"},
    {"name": "PubMed·系统综述", "type": "pubmed", "role": "anchor", "query": "systematic review[pt] AND humans[mh] AND (creatine OR \"omega-3\" OR \"intermittent fasting\" OR \"resistance training\" OR longevity)", "category": "综述", "evidence": "unknown"},

    {"name": "PubMed·间歇禁食RCT", "type": "pubmed", "role": "radar", "query": "intermittent fasting AND randomized controlled trial[pt] AND humans[mh]", "category": "营养", "evidence": "unknown"},
    {"name": "PubMed·肌酸", "type": "pubmed", "role": "radar", "query": "creatine supplementation AND humans[mh]", "category": "补剂", "evidence": "unknown"},
    {"name": "PubMed·Omega-3 RCT", "type": "pubmed", "role": "radar", "query": "omega-3 fatty acids AND randomized controlled trial[pt] AND humans[mh]", "category": "补剂", "evidence": "unknown"},
    {"name": "PubMed·心肺功能与死亡率", "type": "pubmed", "role": "radar", "query": "cardiorespiratory fitness AND mortality AND humans[mh]", "category": "运动", "evidence": "unknown"},

    {"name": "Peter Attia MD", "type": "youtube", "role": "discovery", "group": "attia", "channel_id": "UC8kGsMa0LygSX9nkBcBH1Sg", "category": "长寿", "evidence": "expert"},
    {"name": "Huberman Lab", "type": "youtube", "role": "discovery", "group": "huberman", "channel_id": "UC2D2CMWXMOVWx7giW1n3LIg", "category": "睡眠", "evidence": "expert"},
    {"name": "Huberman Lab Podcast", "type": "rss", "role": "discovery", "group": "huberman", "url": "https://feeds.megaphone.fm/hubermanlab", "category": "睡眠", "evidence": "expert"},
    {"name": "FoundMyFitness", "type": "youtube", "role": "discovery", "group": "fmf", "channel_id": "UC5fdyC4LxyyYv8Am6nDrkmg", "category": "营养", "evidence": "expert"},
    {"name": "FoundMyFitness Podcast", "type": "rss", "role": "discovery", "group": "fmf", "url": "https://podcast.foundmyfitness.com/rss.xml", "category": "营养", "evidence": "expert"},

    {"name": "Bryan Johnson", "type": "youtube", "role": "radar", "group": "bryan", "channel_id": "UCnRVL1-HJnXWB_Xi2dAoTcg", "category": "长寿", "evidence": "blogger"},
    {"name": "Bryan Johnson (Medium)", "type": "rss", "role": "radar", "group": "bryan", "url": "https://bryan-johnson.medium.com/feed", "category": "长寿", "evidence": "blogger"}
  ],
  "_仅作核验依据_不自动抓_在evidence_source_urls按需引用": [
    "NIH ODS 补剂资料库: https://ods.od.nih.gov/factsheets/list-all/",
    ...
    "USPSTF A/B 推荐(标注适用地区): https://www.uspreventiveservicestaskforce.org/",
    "卫健委 健康素养66条 / 中国疾控(本土基线，无 RSS 需手录或 fetch-skill)",
    "中文解释层(丁香医生/卓正/果壳)——只作通俗解释，不替代原始依据"
  ],
  "_未来": [
    "卡片双链接：discovery_source_url(在哪听到) + evidence_source_urls(凭什么核验)——下一专项",
    "ScienceDaily(科学新闻稿)仅作选题雷达，不进正式发布链路"
  ]
}
...
- 在哪听到(discovery)：https://www.hubermanlab.com/episode/build-muscle-great-posture-and-resilience-to-injury-jeff-cavaliere
- 核验依据(evidence)：待标注
- date=2026-05-25/src=2026-05-25/rev=2026-05-31
...
- 在哪听到(discovery)：https://www.hubermanlab.com/episode/how-to-overcome-social-anxiety-nick-epley
- 核验依据(evidence)：待标注
- date=2026-05-18/src=2026-05-18/rev=2026-05-31


### 运动悖论：上班的体力活，可能不像休闲运动那样护心 `physical-activity-paradox`
- `运动`·证据`observational`·精选True(84)·reviewed
- 结论：上班的体力劳动不等于护心运动，休闲运动才可靠降低心血管与死亡风险。
- 适用：未特别限定，见原文
- 注意：证据仍偏弱、属观察性研究，工作体力活动的关联不一致，机制尚不确定，结论需谨慎，见原文核对。
- 在哪听到(discovery)：—
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/42104355/
- date=2026-05-01/src=2026-05-01/rev=2026-05-31


### 任何年纪开始运动都不晚（对心血管） `exercise-any-age-cardiovascular`
- `运动`·证据`observational`·精选False(0)·reviewed
- 结论：任何年纪开始规律运动，都能改善心血管健康。
- 适用：各年龄段，尤其上了年纪、起步较晚者；具体人群见原文。
- 注意：证据为观察性综述（非随机对照），存在因果与混杂局限；具体运动强度、方式与剂量见原文核对。
- 在哪听到(discovery)：—
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/42161438/
- date=2026-05-01/src=2026-05-01/rev=2026-05-31


### 关节活动度，可以不随年龄必然下降 `joint-mobility-aging`
- `关节`·证据`expert`·精选True(86)·reviewed
- 结论：活动度不必随年龄必然下降，但长期不练几乎注定退化。
- 适用：未特别限定，见原文
- 注意：本条为单一专家（Kelly Starrett）观点，重在方法论框架，非临床研究结论；具体方法见原文核对。
- 在哪听到(discovery)：https://foundmyfitness.libsyn.com/111-the-optimal-mobility-protocol-for-a-durable-body-dr-kelly-starrett
- 核验依据(evidence)：待标注
- date=2026-04-24/src=2026-04-24/rev=2026-05-31


### 限时进食能降心梗患者的炎症？一个小型 RCT `intermittent-fasting-mi`
- `营养`·证据`rct`·精选True(82)·reviewed
- 结论：限时进食或降心梗患者炎症指标，属早期信号，别当定论。
- 适用：有心梗病史／已有冠心病的患者（样本仅 19 人）
- 注意：样本很小、仅 2 周、看的是炎症指标而非「少发心梗」等硬结局，属早期信号，需就医个体化评估。
- 在哪听到(discovery)：—
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/41859904/
- date=2026-04-01/src=2026-04-01/rev=2026-05-31
...
- 在哪听到(discovery)：https://foundmyfitness.libsyn.com/110-how-to-build-lasting-happiness-dr-arthur-brooks
- 核验依据(evidence)：待标注
- date=2026-03-24/src=2026-03-24/rev=2026-05-31


### NAD 与衰老：哪些干预真有人体数据 `nad-aging-interventions`
- `长寿`·证据`expert`·精选False(0)·reviewed
- 结论：NAD 补剂要分清「机制说得通」与「真有人体证据」，别照搬。
- 适用：未特别限定，见原文
- 注意：需区分机制合理与人体证据，市面补剂良莠不齐，具体剂量与产品请回看原始研究核对。
- 在哪听到(discovery)：https://foundmyfitness.libsyn.com/109-how-to-boost-nad-levels-to-fight-inflammation-improve-recovery-and-slow-aging-dr-charles-brenner
- 核验依据(evidence)：待标注
- date=2026-02-09/src=2026-02-09/rev=2026-05-31


### 种子油真有那么坏吗？一次把证据捋清 `seed-oils`
- `营养`·证据`expert`·精选True(80)·reviewed
- 结论：种子油未被证明独有害，总热量、纤维、活动量才是关键。
- 适用：未特别限定，见原文
- 注意：饱和脂肪与多不饱和脂肪的权衡缺人体试验定论，仍属争议；专家建议别凭直觉、去读原始研究。
- 在哪听到(discovery)：https://www.youtube.com/watch?v=7_cbaDXAWYM
- 核验依据(evidence)：待标注
- date=2026-01-21/src=2026-01-21/rev=2026-05-31


### 补肌酸会搞坏血脂吗？荟萃说：不会 `creatine-lipids`
- `补剂`·证据`meta`·精选False(0)·reviewed
- 结论：8 项 RCT 荟萃显示肌酸对血脂无临床意义影响。
- 适用：未特别限定，见原文
- 注意：证据确定性偏低，仍需更大样本确认；具体剂量/人群见原文核对。
- 在哪听到(discovery)：—
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/42180567/
- date=2026-01-01/src=2026-01-01/rev=2026-05-31


### 哪种运动对长寿最有用？ `exercise-intensity-longevity`
- `运动`·证据`expert`·精选True(90)·reviewed
- 结论：剧烈运动对延寿或更高效，但属专家梳理而非定论。
- 适用：未特别限定，见原文
- 注意：为专家梳理而非单一 RCT 定论，「1 分钟抵 10 分钟」为节目中说法，建议先听论证再回看原始研究核对。
- 在哪听到(discovery)：https://foundmyfitness.libsyn.com/108-the-best-type-of-exercise-for-longevity
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/36482104/
- date=2025-12-07/src=2025-12-07/rev=2026-05-31


### 失眠：不靠药能怎么改善 `insomnia-cbt-i`
- `睡眠`·证据`expert`·精选False(0)·reviewed
- 结论：CBT-I 与刺激控制是有科学支持的非药物失眠干预手段。
- 适用：慢性失眠及未治疗睡眠呼吸暂停人群；其余未特别限定，见原文。
- 注意：属专家梳理而非随机对照试验结论；严重或长期睡眠问题仍需就医评估，具体方案见原文核对。
- 在哪听到(discovery)：https://foundmyfitness.libsyn.com/107-how-to-cure-insomnia-without-pills-fall-asleep-dr-michael-grandner
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/26054060/
- date=2025-10-02/src=2025-10-02/rev=2026-05-31


### 你的「衰老速度」其实能测 `aging-pace-test`
- `长寿`·证据`observational`·精选False(0)·reviewed
- 结论：有测试能估衰老速度，速度略高于1关联未来7年更高死亡与慢病风险。
- 适用：未特别限定，见原文
- 注意：证据为观察性研究（DunedinPACE 等纵向研究），仅显示相关而非因果；作者「12 个月逆转 31 年」属个人自测、营销色彩重，应当概念看、别当目标。
- 在哪听到(discovery)：https://medium.com/future-literacy/how-fast-are-you-aging-e845830d8a3c
- 核验依据(evidence)：https://pubmed.ncbi.nlm.nih.gov/35029144/
- date=2022-11-08/src=2022-11-08/rev=2026-05-31


### 「每天八杯水」——你怎么知道这是对的？ `eight-glasses-water-how-do-you-know`
- `营养`·证据`blogger`·精选False(0)·reviewed
...
- 在哪听到(discovery)：https://medium.com/future-literacy/how-much-water-should-i-drink-552be7d300e5
- 核验依据(evidence)：待标注
- date=2022-10-07/src=2022-10-07/rev=2026-05-31
